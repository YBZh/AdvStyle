...
class AdvStyle(nn.Module):
    ...
    def __init__(self, p=0.5, alpha=0.1, eps=1e-6, mix='random', channel_num=0, adv_weight=1.0, mix_weight=1.0, **kwargs):
        ...
        print('mix weight is', self.mix_weight)
    ...
    def forward(self, x):
        ...
        #     sig_mu = (var_mu + self.eps).sqrt()
        #     sig_sig = (var_sig + self.eps).sqrt()
        #     self.adv_mean_std.data = sig_mu.detach()
        #     self.adv_std_std.data = sig_sig.detach()
        #     self.moveavg_mean_std = sig_mu.detach()
        #     self.moveavg_std_std = sig_sig.detach()
        #     self.first_flag = False
        #
        # else:
        #     var_mu = mu.var(0, keepdim=True)
        ...
        #     sig_mu = (var_mu + self.eps).sqrt()
        #     sig_sig = (var_sig + self.eps).sqrt()
        #     self.moveavg_mean_std = self.moveavg_mean_std * self.momentum + sig_mu.detach() * (1-self.momentum)
        #     self.moveavg_std_std = self.moveavg_std_std * self.momentum + sig_sig.detach() * (1-self.momentum)
        #
        #     # mixed_adv_mean_std = self.adv_mean_std * self.mix_weight + (1-self.mix_weight) * sig_mu.detach()
        #     # mixed_adv_std_std = self.adv_std_std * self.mix_weight + (1-self.mix_weight) * sig_sig.detach()
        #     # initial_mean_std = torch.randn(mu.size()).cuda()
        #     # initial_std_std = torch.randn(sig.size()).cuda()
        #     # new_mu = initial_mean_std * self.grl_mean(mixed_adv_mean_std, self.adv_weight/self.mix_weight) + mu
        #     # new_sig = initial_std_std * self.grl_std(mixed_adv_std_std, self.adv_weight/self.mix_weight) + sig
        #
        # mixed_adv_mean_std = self.adv_mean_std * self.mix_weight + (1 - self.mix_weight) * self.moveavg_mean_std.detach()
        # mixed_adv_std_std = self.adv_std_std * self.mix_weight + (1 - self.mix_weight) * self.moveavg_std_std.detach()
        # initial_mean_std = torch.randn(mu.size()).cuda()
        # initial_std_std = torch.randn(sig.size()).cuda()
        # new_mu = initial_mean_std * self.grl_mean(mixed_adv_mean_std, self.adv_weight) + mu
        # new_sig = initial_std_std * self.grl_std(mixed_adv_std_std, self.adv_weight) + sig
