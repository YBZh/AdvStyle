        ...
        if self.init_flag:
            ...
            # else:
                ...
                if self.first_flag:
                    ...
                    # ######################################## initialize std with normal distribution.
                    # self.adv_mean_std.data.normal_(0, 1)
                    # self.adv_mean_std.data[self.adv_mean_std.data < 0] = self.adv_mean_std.data[self.adv_mean_std.data < 0] * (-1)
                    # self.adv_std_std.data.normal_(0, 1)
                    # self.adv_std_std.data[self.adv_std_std.data < 0] = self.adv_std_std.data[self.adv_std_std.data < 0] * (-1)
                    # self.first_flag = False
                ...
                # # 只用self.adv_mean_std 和 self.adv_std_std 强度, and current batch std 方向。
                # var_mu = mu.var(0, keepdim=True) ## 1*C*1*1
                # var_sig = sig.var(0, keepdim=True)
                # sig_mu = (var_mu + self.eps).sqrt()  ## 这里提供方向
                # sig_sig = (var_sig + self.eps).sqrt()
                #
                # qiangdu_sig_mu = self.grl_mean(self.adv_mean_std, self.adv_weight)
                # qiangdu_sig_sig = self.grl_std(self.adv_std_std, self.adv_weight)
                #
                # used_sig_mu = sig_mu / torch.norm(sig_mu, p=2,dim=1,keepdim=True) * torch.norm(qiangdu_sig_mu, p=2,dim=1,keepdim=True)
                # used_sig_sig = sig_sig / torch.norm(sig_sig, p=2, dim=1, keepdim=True) * torch.norm(qiangdu_sig_sig, p=2, dim=1,
                #                                                                              keepdim=True)
                # initial_mean_std = torch.randn(mu.size()).cuda()
                # initial_std_std = torch.randn(sig.size()).cuda()
